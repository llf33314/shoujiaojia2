<template>
  <div>
    <el-form-item label="地址导航：">
      <gt-region-choose :selectedOptions="selected" @change="regionChange"></gt-region-choose>
  ...
  export default {
    ...
    methods: {
      ...
      openDialog() {
        ...
        if (this.region2 == '') {
          this.selectedName.forEach((item) => {
            this.region2 += item
          })
        }
          ...
          var init = function () {
            ...
            _this.searchService = new qq.maps.SearchService({
              ...
              complete: function (results) {
                ...
                _this.detailedAddress2 = _this.pois[0]
                // //清除覆盖层
            ...
            clearOverlays()
            //根据输入的城市设置搜索范围
            _this.searchService.setLocation(_this.region2);
            //设置搜索页码
            // _this.searchService.setPageIndex(5);
            //设置每页的结果数
            _this.searchService.setPageCapacity(10);
            //根据输入的关键字在搜索范围内检索
            _this.searchService.search(_this.region2);
          }
          init()
        });
      },
      ...
      changefn() {
        var _this = this;
        this.detailedAddress2 = this.pois[_this.radio]
        //清除覆盖层
        function clearOverlays() {
          if (_this.markers) {
            _this.markers.setMap(null)
      ...
      Determine() {
        ...
        if (this.detailedAddress2.address) {
          this.addressinput = this.detailedAddress2.address
        } else {
          this.addressinput = this.detailedAddress2.name
        }
        this.$emit('update:detailedAddress', this.detailedAddress2);
